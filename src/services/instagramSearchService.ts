...
export class InstagramSearchService {
  ...
  /**
   * 🔍 BUSCA MÚLTIPLA DE PERFIS - IMPLEMENTAÇÃO REAL COM TRANSPARÊNCIA
   * Retorna lista de perfis encontrados para o usuário escolher
   */
  async searchProfiles(query: string, onFallback?: (reason: string) => void): Promise<{
    ...
    try {
      ...
      try {
        ...
        if (realResults.length > 0) {
          ...
          const fallbackReason = 'Nenhum perfil encontrado na busca real';
          console.log(`⚠️ [INSTAGRAM] ${fallbackReason}, usando fallback para "${query}"`);
          
          // Notificar callback sobre fallback
          if (onFallback) {
            onFallback(fallbackReason);
          }
          
          return {
            results: this.generateSimulatedSearchResults(cleanQuery),
            isSimulated: true,
            fallbackReason
          };
        }
      } catch (realSearchError) {
        const fallbackReason = this.getFallbackReason(realSearchError);
        console.error(`❌ [INSTAGRAM] Erro na busca real para "${query}":`, realSearchError);
        console.log(`🔄 [INSTAGRAM] Fallback para simulação: "${query}" - Motivo: ${fallbackReason}`);
   ...
   */
  async searchProfilesLegacy(query: string): Promise<InstagramSearchResult[]> {
    const result = await this.searchProfiles(query);
    return result.results;
  }

  /**
   * 📝 OBTER MOTIVO DO FALLBACK
  ...
  private getFallbackReason(error: any): string {
    ...
    if (errorMessage.includes('CORS') || errorMessage.includes('blocked')) {
      return 'Limitação do navegador (CORS Policy)';
    }
    ...
    if (errorMessage.includes('401') || errorMessage.includes('unauthorized')) {
      return 'Instagram bloqueou a requisição';
    }
    
    return 'Erro técnico na busca real';
  }
