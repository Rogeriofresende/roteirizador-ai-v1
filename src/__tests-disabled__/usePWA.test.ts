...
// jest.MockedFunction da função initializeManifest
jest.mock('../utils/pwa-manifest', () => ({
  initializeManifest: jest.fn(),
}));

// jest.MockedFunction do navigator e window
const mockNavigator = {
  onLine: true,
  serviceWorker: {
    register: jest.fn(),
...
Object.defineProperty(global, 'caches', { value: { open: jest.fn() }, writable: true });
...
describe('usePWA', () => {
  beforeEach(() => {
    ...
    // jest.MockedFunction matchMedia
    ...
    // jest.MockedFunction console métodos
  ...
  describe('Função showInstallPrompt', () => {
    it('mostra prompt quando disponível', () => {
      ...
      // jest.MockedFunction alert para instruções manuais
