/**
 * 🛡️ ERROR BOUNDARY V6.3
 * Enhanced React Error Boundary with Error Capture integration
 */

import React, { Component, ErrorInfo, ReactNode } from 'react';
import { logger } from '../utils/logger';
import { config } from '../config/environment';
import { sendErrorToMonitoring } from '../utils/errorCapture';
...
export class ErrorBoundaryV6 extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    // V6.3: Send to error capture system
...
export function withErrorBoundaryV6<P extends object>(
  Component: React.ComponentType<P>,
  errorBoundaryProps?: Omit<Props, 'children'>
) {
  const WrappedComponent = (props: P) => (
    <ErrorBoundaryV6 {...errorBoundaryProps}>
      <Component {...props} />
    </ErrorBoundaryV6>
  );

  WrappedComponent.displayName = `withErrorBoundaryV6(${Component.displayName || Component.name})`;
