...
describe('ScriptForm', () => {
  ...
  it('gera roteiro com sucesso', async () => {
    ...
    GeminiService.generateScript.mockResolvedValue(mockScript);
    ...
    // Aguarda conclusão
    await waitFor(() => {
      expect(GeminiService.generateScript).toHaveBeenCalledWith({
  ...
  it('trata erro na geração do roteiro', async () => {
    GeminiService.generateScript.mockRejectedValue(new Error('Erro na API'));
  ...
  it('desabilita botão durante carregamento', async () => {
    GeminiService.generateScript.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000)));
